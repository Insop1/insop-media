...
pub fn run_event_loop(config: &Config, cache_dir: &Path) -> Result<()> {
    ...
    let mut child = command.spawn().context("playerctl not found")?;
    ...
    let tx = if config.images {
        ...
        thread::spawn(move || {
            for url in rx {
                if let Err(e) = download_url_image(&url, &thread_cache_dir) {
                    eprintln!("{e:#}");
                }
            }
        });
...
fn download_url_image(url: &str, cache_dir: &Path) -> Result<()> {
    let image_path = cache_dir.join("current.jpg");
    let tmp_path = cache_dir.join("current.jpg.tmp");

    let response = ureq::get(url)
        .call()
        .context("Failed to download url image")?;
    let bytes = response
        .into_body()
        .read_to_vec()
        .context("Failed to read bytes")?;

    fs::write(&tmp_path, &bytes).context("Failed to write to tmp file")?;
